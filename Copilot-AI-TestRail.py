...
def get_llm_client() -> LLMClient:
    ...
    if _llm_client is None:
        ...
        _llm_client.log_provider(logger=logger)
    return _llm_client
...
async def refactor_case_with_agent(raw_case: dict) -> dict:
    ...
    prompt_payload = {
        ...
        "agent_guidance": MAIN_AGENT_TEXT,
        "output_schema": RefactoredTestCase.model_json_schema(),
        "input_case": {
            "title": safe_str(raw_case.get("title")),
            "preconditions": safe_str(raw_case.get("custom_preconds")),
            "steps_text": safe_str(raw_case.get("custom_steps")),
            "expected_text": safe_str(raw_case.get("custom_expected")),
            "steps_separated": raw_case.get("custom_steps_separated") or [],
        },
    }
    ...
    try:
        ...
        validated = RefactoredTestCase(**parsed)
        return validated.model_dump()
    except (json.JSONDecodeError, ValidationError, ValueError, RuntimeError, EnvironmentError, aiohttp.ClientError) as err:
        log(f"[REFAC][LLM] Fallback to local refactor due to error: {err}")
        return refactor_case_locally(raw_case)
